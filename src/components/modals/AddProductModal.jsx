...
const AddProductModal = ({ isOpen, toggle, getProducts }) => {
  ...
  const intialState = { loading: false, name: '', nameError: '', image: '', imageUrl: '', imageError: '', content: '', addProductError: '', note: '' }
  ...
  const addProduct = async () => {
    const { name, nameError, image, imageError, content, addProductError, note } = state;

    const isValid = name && content && image && (nameError === intialState.nameError) && (imageError === intialState.imageError) && (addProductError === intialState.addProductError);
    if (!isValid) {
